...
#define _DUTY_NEU 900  // Servo angle: 0 degree
...
void setup() {
  ...
  dist_target = 50; // 15.5 cm, the center of the rail
}
...
void loop() {
  ...
  if(event_dist) {
    ...
    // bang bang control
    if(dist_ema<50) { // Replace 1 with a proper test expression using dist_target and dist_ema.
...
// Replace the below line with the equation obtained from nonlinear regression analysis
  return 746 + (-2.66*x) + 2.35*(1/1000)*x*x;
}
